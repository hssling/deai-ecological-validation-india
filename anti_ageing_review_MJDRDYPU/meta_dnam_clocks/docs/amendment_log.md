...
### A3 — Adjusted between-group effects accepted alongside SMD
- **Original synthesis plan (protocol §3.1):** Effect measure = Hedges' g standardized mean difference of change from baseline (intervention vs control), computed from arm-stratified means and SDs.
- **Amendment:** When a study reports an *adjusted between-group mean difference* on the original clock scale (e.g., "intervention vs control, β = -0.42 years, 95% CI -0.81 to -0.03" for an age clock, or analogous β for DunedinPACE), accept that as the primary effect estimate. Convert CI to SE via `SE = (CI_upper - CI_lower) / (2 * 1.96)`. Pool via **generic-inverse-variance random-effects** on the original scale (mean difference in years for age clocks; per-year-per-year for DunedinPACE). Where Hedges' g is also extractable, both are reported (primary = MD on original scale; sensitivity = SMD).
- **Reason:** Most published DNAm-clock RCTs and prospective trials report adjusted regression coefficients (β with SE or 95% CI) for the intervention indicator rather than arm-stratified means/SDs. The earlier extractor only captured arm-level means and therefore missed nearly every published headline effect. This amendment captures the published effect at the scale and adjustment level used by the source authors.
- **Operational rule:** Only emit a candidate effect if (a) the clock identity is unambiguous, (b) the effect is clearly an intervention/exposure vs control between-group estimate (not autocorrelation, within-arm change, or baseline-only difference), and (c) the numeric values are read literally from the source. Uncertain extractions are flagged for human review.
- **Implementation:** `src/24_extract_adjusted_effects.py`; downstream pooling in `src/08_meta_analysis.py` switches to generic-inverse-variance RE (DL primary, HKSJ sensitivity, bayesmeta sensitivity) with an R cross-check via `r_scripts/meta_pool.R`.
